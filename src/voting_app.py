...
class VotingApp:
    ...
    def setup_voting_tab(self):
        ...
        control_frame = ttk.LabelFrame(self.voting_tab,
                        text="Create Vote")
        control_frame.pack(fill=tk.X, expand=False, padx=10, pady=10)
        
        # Vote option
        ttk.Label(control_frame, text="Candidate:").grid(
                        row=0, column=0, padx=5, pady=5, sticky=tk.W)
        self.vote_option_var = tk.StringVar()
        
        # Predefined options for this demo
        options = ["Candidate A", "Candidate B", "Candidate C"]
        self.vote_option_combobox = ttk.Combobox(control_frame,
                        textvariable=self.vote_option_var, values=options)
        self.vote_option_combobox.grid(
                        row=0, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Vote button
        self.vote_button = ttk.Button(control_frame, text="Cast Vote",
                        command=self.cast_vote)
        self.vote_button.grid(row=0, column=2, padx=5, pady=5)
        
        # Mine button
        self.mine_button = ttk.Button(control_frame, text="Mine Block",
        ...
        self.auto_mine_check = ttk.Checkbutton(
            control_frame, text="Auto-Mine", variable=self.auto_mine_var,
                command=self.toggle_auto_mine
        )
        self.auto_mine_check.grid(row=0, column=4, padx=5, pady=5)
        
        # Frame for transaction log
        log_frame = ttk.LabelFrame(self.voting_tab,
                        text="Transaction Log")
        log_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Transaction log
        self.transaction_log = scrolledtext.ScrolledText(log_frame,
    ...
    def setup_blockchain_tab(self):
        ...
        info_frame = ttk.LabelFrame(self.blockchain_tab,
        ...
        ttk.Label(info_frame, textvariable=self.chain_length_var).grid(
                        row=0, column=0, padx=5, pady=5, sticky=tk.W)
        
        self.last_block_var = tk.StringVar(value="Last Block: None")
        ttk.Label(info_frame, textvariable=self.last_block_var).grid(
                        row=1, column=0, padx=5, pady=5, sticky=tk.W)
        
        self.pending_tx_var = tk.StringVar(
                        value="Pending Transactions: 0")
        ttk.Label(info_frame, textvariable=self.pending_tx_var).grid(
                        row=2, column=0, padx=5, pady=5, sticky=tk.W)
        
        self.mining_status_var = tk.StringVar(value="Mining: No")
        ttk.Label(info_frame, textvariable=self.mining_status_var).grid(
                        row=3, column=0, padx=5, pady=5, sticky=tk.W)
        
        # Frame for blockchain display
        chain_frame = ttk.LabelFrame(self.blockchain_tab,
                        text="Blockchain")
        chain_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Treeview for blocks
        self.block_tree = ttk.Treeview(chain_frame, columns=(
            "Index", "Hash", "Prev Hash", "Timestamp", "Transactions"))
        ...
        self.block_tree.column(
                        "Timestamp", width=150, anchor=tk.CENTER)
        self.block_tree.column(
                        "Transactions", width=100, anchor=tk.CENTER)
        
        # Add scrollbar
        scrollbar = ttk.Scrollbar(chain_frame, orient=tk.VERTICAL,
    ...
    def setup_results_tab(self):
        ...
        # Frame for results
        results_frame = ttk.LabelFrame(self.results_tab,
                        text="Voting Results")
        ...
        refresh_button = ttk.Button(button_frame,
            text="Refresh Results", command=self.refresh_results)
    ...
    def setup_network_tab(self):
        ...
        info_frame = ttk.LabelFrame(
                        self.network_tab, text="Network Information")
        info_frame.pack(fill=tk.X, expand=False, padx=10, pady=10)
        
        # Client ID
        self.client_id_var = tk.StringVar(
                        value=f"Client ID: {self.client.id}")
        ttk.Label(info_frame, textvariable=self.client_id_var).grid(
                        row=0, column=0, padx=5, pady=5, sticky=tk.W)
        
        # Tracker info
        self.tracker_info_var = tk.StringVar(
                        value=f"Tracker: {self.client.tracker_host}:{self.client.tracker_port}")
        ttk.Label(info_frame, textvariable=self.tracker_info_var).grid(
                        row=1, column=0, padx=5, pady=5, sticky=tk.W)
        
        # Peer count
        self.peer_count_var = tk.StringVar(value="Peers: 0")
        ttk.Label(info_frame, textvariable=self.peer_count_var).grid(
                        row=2, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        # Treeview for peers
        self.peer_tree = ttk.Treeview(peers_frame,
        ...
        # Add scrollbar
        scrollbar = ttk.Scrollbar(peers_frame, orient=tk.VERTICAL,
                        command=self.peer_tree.yview)
    ...
    def cast_vote(self):
        ...
        if success:
            ...
            messagebox.showinfo("Success", f"Your vote for " +
                            f"{option} has been cast")
            self.add_to_transaction_log(f"Vote cast for {option}")
        else:
            self.status_var.set(f"Failed to cast vote")
            messagebox.showerror("Error", f"Failed to cast vote. " +
                            f"Have you already voted?")
    ...
    def toggle_auto_mine(self):
        ...
        self.status_var.set(f"Auto-mining: " +
                        f"{'On' if self.client.auto_mine else 'Off'}")
    ...
    def add_to_transaction_log(self, message):
        ...
        self.transaction_log.insert(tk.END,
    ...
    def update_blockchain_info(self):
        ...
        # Update display
        self.chain_length_var.set(
                        f"Chain Length: {info['chain_length']}")
        self.last_block_var.set(f"Last Block: " +
                        f"{info['last_block_hash'][:10]}... (truncated)")
        self.pending_tx_var.set(f"Pending Transactions: " +
                        f"{info['pending_transactions']}")
        self.mining_status_var.set(
                        f"Mining: {'Yes' if info['mining'] else 'No'}")
    ...
    def update_peer_list(self, peers):
        ...
        for peer_id, (host, port) in peers.items():
            self.peer_tree.insert("", tk.END,
                            values=(peer_id, host, port))
    ...
    def show_block_details(self, event):
        ...
        # Block info
        ttk.Label(info_tab, text="Index:").grid(
                        row=0, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=str(block.index)).grid(
                        row=0, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Hash:").grid(
                        row=1, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=block.hash).grid(
                        row=1, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Previous Hash:").grid(
                        row=2, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=block.previous_hash).grid(
                        row=2, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Timestamp:").grid(
                        row=3, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=time.strftime("%Y-%m-%d %H:%M:%S",
                        time.localtime(block.timestamp))).grid(
                        row=3, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Nonce:").grid(
                        row=4, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=str(block.nonce)).grid(
                        row=4, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Merkle Root:").grid(
                        row=5, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=block.merkle_root).grid(
                        row=5, column=1, padx=5, pady=5, sticky=tk.W)
        
        ttk.Label(info_tab, text="Transaction Count:").grid(
                        row=6, column=0, padx=5, pady=5, sticky=tk.W)
        ttk.Label(info_tab, text=str(len(block.transactions))).grid(
                        row=6, column=1, padx=5, pady=5, sticky=tk.W)
        
        # Transactions
        tx_tree = ttk.Treeview(tx_tab,
                        columns=("ID", "Voter", "Choice", "Timestamp"))
        ...
        # Add scrollbar
        scrollbar = ttk.Scrollbar(tx_tab, orient=tk.VERTICAL,
                        command=tx_tree.yview)
