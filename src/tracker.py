...
import json
import sys
from network import (Network, MSG_REGISTER, MSG_PEER_LIST,
                MSG_HEARTBEAT, MSG_CHAIN_RESPONSE, MSG_GET_MINER,
                MSG_VOTE_RESULTS)
...
class Tracker(Network):
    ...
    def __init__(self, host: str, port: int,
        ...
        self.assigned_ids = 0
        self.peers = {}
        # {peer_id: (host, port, last_heartbeat, stake_value)}
        self.active_peers = {}
        self.miners = {}
        self.vote_results = {}
    ...
    def _handle_register(self, message: Dict[str, Any]) -> None:
        ...
        if 'data' in message and 'sender' in message:
            ...
            if 'host' in peer_data and 'port' in peer_data:
                ...
                logger.info(f"Registered peer " +
                                f"{peer_id} at {host}:{port}")
    ...
    def _handle_heartbeat(self, message: Dict[str, Any]) -> None:
        ...
        if 'sender' in message:
            ...
            if peer_id in self.active_peers:
                ...
                if 'data' in message and \
                                'blockchain' in message['data']:
                    ...
                    temp_chain_check, miner_results = \
                                    temp_chain.is_chain_valid()
                    if (temp_chain_check and
                                    len(temp_chain.chain) >= \
                                        len(self.blockchain.chain)):
                        if (len(temp_chain.chain) > \
                                        len(self.blockchain.chain) or 
                                        temp_chain.chain_score > \
                                        self.blockchain.chain_score):
                            self.blockchain = temp_chain
                            logger.info(f"Updated reference blockchain"
                                            + f" from peer {peer_id}")
                            # Broadcast vote results if there are new results
                            vote_results = \
                                self.blockchain.get_vote_results()
                            has_new_votes = \
                                self.merge_vote_results(vote_results)
    ...
    def _check_heartbeats(self) -> None:
        ...
        while self.running:
            ...
            for peer_id, (host, port, last_heartbeat) in \
            ...
            for peer_id in dead_peers:
                host, port, _ = self.active_peers[peer_id]
                logger.info(f"Removing dead peer " +
                                f"{peer_id} at {host}:{port}")
    ...
    def _broadcast_peer_list(self) -> None:
        ...
        logger.info(f"Broadcast peer list to " +
                        f"{len(self.active_peers)} peers")
    ...
    def _handle_get_miner(self, message: Dict[str, any]):
        if 'sender' in message:
            ...
            if peer_id in self.active_peers:
                ...
                self.send_message((host, port), message)
                logger.info(f"Sent miner id and stake value to " +
                                f"{peer_id}")
    ...
    def _broadcast_vote_results(self):
        ...
        logger.info(f"Broadcast voting results to " +
                        f"{len(self.active_peers)} peers")
