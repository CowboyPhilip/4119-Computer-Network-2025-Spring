...
def main():
    ...
    for i in range(args.peers):
        ...
        # Start the peer with the web voting GUI
        ...
        peer_process = subprocess.Popen(
            [
                sys.executable, "web_voting_app.py", 
                peer_host, str(peer_port), 
                tracker_host, str(tracker_port),
                "topology_web.dat", str(args.difficulty), auto_mine_arg
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )
        processes.append(peer_process)
        webbrowser.open_new_tab(peer_host + ":" + str(peer_port))
