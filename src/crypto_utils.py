...
def generate_key_pair() -> Tuple[str, str]:
    ...
    private_key = hashlib.sha256(
                    (base_id + str(time.time())).encode()).hexdigest()
...
def sign_message(message: str, private_key: str) -> str:
    ...
    return hashlib.sha256(
                    (message + private_key).encode()).hexdigest()
...
def verify_signature(message: str, signature: str,
    ...
    expected_format = hashlib.sha256(
                    (message + get_private_key_for_demo(public_key)
                                    ).encode()).hexdigest()
...
def get_private_key_for_demo(public_key: str) -> str:
    ...
    return hashlib.sha256(
                    ("DEMO_ONLY_" + public_key).encode()).hexdigest()