...
def main():
    ...
    parser = argparse.ArgumentParser(
                    description='Start a blockchain voting network.')
    
    parser.add_argument('--peers', type=int, default=3,
                    help='Number of peers to start (default: 3)')
    parser.add_argument('--auto-mine', action='store_true',
                    help='Enable auto-mining on peers')
    ...
    tracker_process = subprocess.Popen(
        [sys.executable, "tracker.py", tracker_host, str(tracker_port),
                        "topology.dat"],
    ...
    for i in range(args.peers):
        ...
        # Give each peer time to start to avoid overwhelming the tracker
        time.sleep(1)
